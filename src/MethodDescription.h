 ...
 */
#ifdef VSCODE
#define INF_REGISTER_FUNC(func, ...) \
    static Inf::MethodDescription func##_desc(s_PBNI_ClassName, &Inf_PBNI_Class::func, __VA_ARGS__)
#else
#define INF_REGISTER_FUNC(func, name, ...) \
    static Inf::MethodDescription func##_desc(s_PBNI_ClassName, &Inf_PBNI_Class::func, name __VA_OPT__(, ) __VA_ARGS__)
#endif
...
namespace Inf
{
    ...
    public:
         ...
         * This will check whether the Argument Types are correct, gather the Arguments, call the Method, set References and
         * ReturnValue
         *
    ...
    protected:
        ...
        /**
         * This Function is there so parameter unpacking can be done with a tuple, it just calls SetValue if Arg is a
         * reference.
         *
         * \param session   Current Session
            ...
            if constexpr (std::is_reference_v<Arg>)
            {
                ...
                if (res != PBX_SUCCESS)
                    throw PBNI_PowerBuilderException(
                        L"IPB_Value::Set<Type> for " + Type<std::remove_reference_t<Arg>>::GetPBName(L""), res
                    );
            }
        }
    };
    ...
    public:
        ...
        template<typename... ArgNames>
            requires(
                sizeof...(Args) == sizeof...(ArgNames) &&
                (std::is_same_v<std::remove_extent_t<ArgNames>, const wchar_t> && ...)
            )
        MethodDescription(
            ...
            if constexpr (sizeof...(Args) > 0)
            {
                ...
                (
                    [&]
                    {
                        ...
                        {
                            m_Description += L"ref " +
                                             Type<std::remove_reference_t<Args>>::GetPBName(std::wstring(L" ") + arg_names) +
                                             L", ";
                        }
                        else
                        {
        ...
        PBXRESULT Invoke(PBNI_Class* object, IPB_Session* session, PBCallInfo* ci) override
        {
            ...
            (
                [&]
                {
                    ...
                    if (!value.Is<std::remove_reference_t<Args>>() ||
                        std::is_reference_v<Args> != (ci->pArgs->GetAt(i)->IsByRef() != 0))
            ...
            // Gathering arguments
            i = 0;
            std::tuple<Cls*, std::remove_reference_t<Args>...> args{
                static_cast<Cls*>(object),
                Helper::PBValue(session, ci->pArgs->GetAt(i++)).Get<std::remove_reference_t<Args>>(false)...
            };
