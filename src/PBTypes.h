...
namespace Inf
{
    ...
    struct Type
    {
        // This Field is used to build a Method Signature, if you want to invoke a PowerBuilder Function from C++.  static
        // constexpr wchar_t PBSignature = L' '; // This Field is used to check for correct IPB_Value Types.
        static constexpr pbvalue_type PBType = pbvalue_notype;

        /**
         * This Function is used to generate the Description of a Method, it needs to take in the name of the argument so we
         * can add [] at the end of arrays. This wont add any spaces between the type and argument_name.
         *
...
#pragma region Bytelike
    ...
    struct Type<PBBoolean>
    {
        ...
        static inline std::wstring GetPBName(const std::wstring& argument_name)
        {
            return L"boolean" + argument_name;
        }
    };
...
#pragma region Date_Times
    ...
    struct Type<PBDateTime>
    {
        ...
        static inline std::wstring GetPBName(const std::wstring& argument_name)
        {
            return L"datetime" + argument_name;
        }
    };
#pragma endregion
...
#pragma region Variable_Sized
    ...
    struct Type<PBBlob>
    {
        ...
        static inline std::wstring GetPBName(const std::wstring& argument_name)
        {
            return L"blob" + argument_name;
        }
    };

    /**
     * For Types that have their own template types (PBObject<>, PBArray<>) we cant partially specialize the static
     * Functions. Instead we have to create our own Struct, which can be partially specialized
     */
    ...
    struct Type<DynPBObject>
    {
        ...
        static inline std::wstring GetPBName(const std::wstring& argument_name)
        {
            return L"powerobject" + argument_name;
        }
    };
    ...
    struct Type<PBObject<class_id, group_type>>
    {
        ...
        static inline std::wstring GetPBName(const std::wstring& argument_name)
        {
            return std::wstring(class_id.data) + argument_name;
        }
    };
